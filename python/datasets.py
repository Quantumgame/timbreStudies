...
def load_timbrespace_features(timbrespace,
        ...
        for root, dirs, files in os.walk(timbrespace_db[timbrespace]['path']):
            for name in files:
                if name.split('.')[-1] in ['aiff', 'wav']:
                    ...
                    if (rs == 'strf'):
                        if window==None:
                            timbrespace_features[rs].append(features.strf(audio, **strf_params))
                        else:
    ...
    if (verbose):
        print('  |_ num. of sounds: {}'.format(len(timbrespace_features[representations[0]])))
        ...
        for root, dirs, files in os.walk(timbrespace_db[timbrespace]['path']):
            for name in files:
                if name.split('.')[-1] in ['aiff', 'wav']:
                    ...
                    num_frames = int((len(audio) - win_length_n) / hop_length_n)
                    windowed_features = []
                    for wn in range(num_frames):
                        start_n = wn * win_length_n
                        end_n = wn * win_length_n + hop_length_n
